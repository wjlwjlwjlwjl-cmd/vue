...
> 前面讲的 ref 是为组件提供响应式数据，这里的 ref 是组件的属性，用来获取一个组件的 DOM 对象

1. 获取响应式数据
2. 组件 ref 属性绑定到响应式数据
3. ref.value 获取到 DOM 对象，之后可以直接对 DOM 对象直接操作 

```html
<template>
	<div ref='divRef'>这是一个div标签</div>
</template>
<script setup>
	import { ref } from 'vue'
...
获取到对象 DOM 对象后，可以直接操作 css，另一方面也可以直接调用对象的方法。需要将子组件的方法、属性暴露出来，可以使用 `defineExpose({})`
...
在 Vue 中，当响应式数据发生改变时，使用响应式数据的组件并不会立刻更新，而是存在一个队列进行缓存，统一进行更新。如果想要获取更新过的 DOM，就可以使用 nextTick()。使用方式有两种：

1. `async + await nextTick()`，后续再获取的 DOM 对象就是更新过的
2. `nextTick(() => {})`，直接在回调函数中对更新过的 DOM 对象进行操作
...
1. 在 main.js 中导入，其中 el 即为被指令修饰对象的 DOM，mounted 会在对象被挂载时自动执行，updated 在对象更新时自动执行

```js
import App from './App.vue'
import { createApp } from 'vue'
const app = App.createApp()
app.directive('instuctionName', {
	mounted(el){
		//codes
	},
	updated(el){
...
2. 完成定义后，可以直接通过 `v-instructionName` 的方式使用指令

**绑定数据**

我们希望自己的指令能够通过 `v-instructionName="data"` 的方式进行数据操作，可以在自定义指令时在参数里加上数据
...
大多数情况下，我们其实期望指令在挂载和更新时的指令操作是相同的，所以可以只传一个回调，进行相同的行为
...
* **默认插槽**，直接在组件中通过 `<slot></slot>` 的方式占位即可；父组件使用时，通过 `<subItem><div></div></subItem>` 的双标签形式，将需要在插槽中呈现的标签结构传入即可

> 插槽也可以提供默认内容，直接在 `<slot></slot>` 标签中搭建默认内容即可，如果父组件处没有提供结构，就会使用默认的标签结构展示

* **具名插槽**，通过给 `<slot name="slotName"> </slot>` 的方式给插槽命名；父组件处，通过 `<template #slotName> <subItem> <div></div> </subItem> </template>` 的方式将标签结构直接传入对应的插槽（这样就允许了一个组件可以提供多个插槽供父组件自定义）
* **作用域插槽**，让插槽能够携带数据，父组件处通过 `<template #slotName="data"></template>` 的 data 变量来接受子组件通过插槽带来的数据，默认为 `{}`；子组件通过插槽基于数据，通过自定义属性的方式，例如 `<slot name="slotName" :a="111" :b="222"> </slot>`

## 八、vue-router，vue 路由管理
